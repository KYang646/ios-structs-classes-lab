...
```
```
There is an error because homePlanet was declared as a constant and constants are not mutable.
```
...
```
```
No because the constant bilbo declared those traits
```
...
```
edgar.name and jason.name will be "Jason" because it was the last name to be recorded
```
...
```
It would have no value because the code will not run.
```
